<template>
    <div style="overflow-x: auto;">
        <div style="min-width: 700px;">
            ...
            <div v-if="!groupedSales || groupedSales.length === 0" class="text-center text-muted py-4">
                No data available.
            </div>
            <div v-for="group in groupedSales" :key="group.invoice_no" class="card mb-3 shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center py-2">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="btn-group btn-group-sm" role="group">
                            ...
                            <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'cashier' && auth.user?.role?.name !== 'warehouse staff'" type="button" class="btn btn-warning" @click="emit('edit-sale', group)" title="Edit invoice"><i class="bi bi-pencil-square"></i></button>
                            <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'cashier' && auth.user?.role?.name !== 'manager' && auth.user?.role?.name !== 'warehouse staff'" class="btn btn-danger" @click="emit('delete-sale', group)"><i class="bi bi-trash" title="Delete invoice"></i></button>
                        </div>
                    </div>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive mb-0">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                <tr v-for="sale in group.items" :key="sale.id" style="font-size: 12px;">
                                    <td class="text-start">
                                        <div><i class="bi bi-bag-fill"></i> {{ sale.product?.name || '-' }}</div>
                                        <div v-if="sale.attributes && typeof sale.attributes === 'object'" class="mt-1">
                                            <template v-if="Array.isArray(sale.attributes)">
                                                <span v-for="grp in groupedAttrs(sale.attributes)" :key="grp.key" class="badge bg-light text-dark me-1 badge-attr">
                                                    {{ grp.key }}: {{ grp.values.join(', ') }}
                                                </span>
                                            </template>
                                            <template v-else>
                                                <span v-for="(v, k, i) in sale.attributes" :key="k" class="badge bg-light text-dark me-1 badge-attr">
                                                    {{ k }}: {{ v }}
                                                </span>
                                            </template>
                                        </div>
                                        <div><i class="bi bi-box-fill"></i> Unit: {{ sale.unit?.name || '-' }}</div>
                                        <div><i class="bi bi-person-fill"></i> {{ sale.client?.name || sale.client?.contact || '-' }}</div>
                                        <div><i class="bi bi-building-fill"></i> {{ sale.branch?.name || '-' }}</div>
                                        <div><i class="bi bi-person-badge-fill"></i> {{ sale.user?.name || '-' }}</div>
                                        <div v-if="sale.updated_by"><i class="bi bi-person-fill"></i> {{ sale.updated_by?.name || '-' }}</div>
                                    </td>
                                    <td class="text-start">
                                        <div><i class="bi bi-currency-dollar"></i> Price: {{ sale.price || '-' }}</div>
                                        <div><i class="bi bi-file-earmark-text-fill"></i> Vat: {{ sale.vat || '-' }}</div>
                                        <div><i class="bi bi-bookmark-dash-fill"></i> Discount: {{ sale.discount || '-' }}</div>
                                    </td>
                                    <td>{{ sale.qty || '-' }}</td>
                                    <td>{{ ((sale.qty * sale.price) + (sale.qty * (sale.vat || 0)) - (sale.qty * (sale.discount || 0))) || '-' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
<script setup>
    ...
    const props = defineProps({
        groupedSales: { type: Array, default: () => [] },
    ...
    const emit = defineEmits(['edit-sale', 'delete-sale', 'page-change', 'per-page-change', 'reload', 'export-started']);
    ...
    const allSelected = computed(() =>
        props.groupedSales.length > 0 && props.groupedSales.every(g => selectedInvoices.value.has(g.invoice_no))
    );

    const toggleSelectAll = () => {
        if (allSelected.value) {
            props.groupedSales.forEach(g => selectedInvoices.value.delete(g.invoice_no));
            selectedInvoices.value = new Set(selectedInvoices.value);
        } else {
            const next = new Set(selectedInvoices.value);
            props.groupedSales.forEach(g => next.add(g.invoice_no));
    ...
    const showingText = computed(() => {
        ...
        if (!pagination.total || pagination.total === 0) return 'No sales to show'

        const from = pagination.from || 1
        const to = pagination.to || pagination.data?.length || 0
        const total = pagination.total || 0

        return `Showing ${from} to ${to} of ${total} sales`
    });
