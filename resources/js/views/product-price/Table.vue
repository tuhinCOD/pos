<template>
    <div class="d-flex align-items-center justify-content-between mb-3">
        <div class="page-item d-flex align-items-center me-1">
            ...
            <ExportButton v-if="auth.user?.role?.name !== 'super admin'" :asyncExport="true" :endpoint="exportEndpoint" :filters="exportFilters" @export-started="emit('export-started', $event)" />
        </div>
        <div class="col-6">
            <input
                class="form-control"
        ...
        <table class="table table-bordered table-striped">
            <thead>
                <tr class="text-center">
                    ...
                    <th class="col-3">Prices</th>
                    <th class="col-1" v-if="auth.user?.role?.name !== 'super admin'">Action</th>
                </tr>
            </thead>
            <tbody id="list">
                <tr v-for="pp in productPrices" :key="pp.id" class="text-center align-middle" style="font-size: 12px">
                    <td><input type="checkbox" :checked="selectedIds.has(pp.id)" @change="toggleSelect(pp.id)" /></td>
                    <td style="font-size: 12px;" class="text-start">
                        <div class="about-product">
                            ...
                            <div>
                                ...
                                Unit: {{ pp.product?.unit?.name || '-' }}<br>
                            </div>
                            <div>
                                <i class="bi bi-envelope-fill"></i>
                                Remarks: {{ pp.remarks || '-' }}<br>
                            </div>
                            <div>
                                <i class="bi bi-person-fill"></i>
                                Created by: {{ pp.user?.name || '-' }}<br>
                            </div>
                            <div>
                                <i class="bi bi-person-fill"></i>
                                updated by: {{ pp.updated_by?.name || '-' }}<br>
                            </div>
                        </div>
                    </td>
                    <td style="font-size: 12px;" class="text-start">
                    ...
                    <td v-if="auth.user?.role?.name !== 'super admin'">
                        <div class="btn-group btn-group-sm" role="group">
                            <button type="button" class="btn btn-warning edit-btn" @click="emit('edit-price', pp)"><i class="bi bi-pencil-square"></i></button>
                            <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'manager'" class="btn btn-danger delete-btn" @click="emit('delete-price', pp.id)"><i class="bi bi-trash"></i></button>
                        </div>
                    </td>
                </tr>
                <tr v-if="productPrices.length === 0">
    ...
    <nav aria-label="Page navigation">
        <div class="d-flex align-items-center justify-content-between">
            ...
            <li class="page-item d-flex align-items-center gap-2 ms-3" v-if="pagination?.last_page > 1">
        ...
        <ul class="pagination justify-content-center m-0 mt-3" v-if="pagination?.last_page > 1">
            ...
            <li class="page-item" :class="{ disabled: pagination.current_page === pagination.last_page }">
                <button v-if="auth.user?.role?.name !== 'super admin'" class="page-link" @click="emit('page-change', pagination.current_page + 1)">
                    Next
                </button>
            </li>
        </ul>
...
<script setup>
    ...
    const allSelected = computed(() =>
        props.productPrices.length > 0 && props.productPrices.every(item => selectedIds.value.has(item.id))
    );
    ...
    const confirmExportSelected = async () => {
        ...
        if (confirm(`Are you sure you want to download ${ids.length} data?`)) {
            try {
                const { data } = await axios.get(`/api/${props.exportEndpoint}/export?ids=${ids.join(',')}`);
                clearSelection();
                emit('export-started', data.file);
            } catch {
                alert('Export failed');
            }
        }
    ...
    const toggleSelectAll = () => {
        if (allSelected.value) {
            props.productPrices.forEach(item => selectedIds.value.delete(item.id));
            selectedIds.value = new Set(selectedIds.value);
        } else {
            const next = new Set(selectedIds.value);
            props.productPrices.forEach(item => next.add(item.id));
    ...
    const showingText = computed(() => {
        ...
        if (!pagination.total || pagination.total === 0) return 'No prices to show'

        const from = pagination.from || 1
        const to = pagination.to || pagination.data?.length || 0
        const total = pagination.total || 0

        return `Showing ${from} to ${to} of ${total} prices`
    })
</script>
