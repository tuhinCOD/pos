<template>
    <div class="table-responsive">
        ...
        <table class="table table-bordered table-striped" style="min-width: 576px;">
            ...
            <tbody id="list">
                <tr v-if="!damages || damages.length === 0">
                    <td colspan="6" class="text-center text-muted">
                        No data available.
                    </td>
                </tr>
                <tr v-for="item in props.damages" :key="item.id" class="text-center align-middle" style="font-size: 12px">
                    ...
                    <td style="font-size: 12px;" class="text-start">
                        <div class="about-damage">
                            ...
                            <div v-if="item.attributes && Object.keys(item.attributes).length">
                                ...
                                Attributes:
                                <span v-for="(v, k) in item.attributes" :key="k" class="badge bg-light text-dark me-1">{{ k }}: {{ v }}</span>
                            </div>
                    ...
                    <div class="btn-group btn-group-sm" role="group" aria-label="Basic mixed styles example">
                        <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'cashier'" type="button" class="btn btn-warning edit-btn" @click="emit('edit-damage', item)" title="Edit damage"><i class="bi bi-pencil-square"></i></button>
                        <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'cashier' && auth.user?.role?.name !== 'manager' && auth.user?.role?.name !== 'warehouse staff'" class="btn btn-danger delete-btn" @click="emit('delete-damage', item)"><i class="bi bi-trash" title="Delete damage"></i></button>
                    </div>
                    </td>
                </tr>
            </tbody>
            ...
            <nav aria-label="Page navigation">
                <div class="d-flex flex-nowrap align-items-center justify-content-between gap-3">
                    ...
                    <div class="d-flex justify-content-end mb-2">
                        <ExportButton v-if="auth.user?.role?.name !== 'super admin'" :endpoint="exportEndpoint" :filters="exportFilters" :requireDays="true" :asyncExport="true" @export-started="(file) => emit('export-started', file)" />
                    </div>
                </div>
            </nav>
        </div>
...
<script setup>
    ...
    const emit = defineEmits(['edit-damage', 'delete-damage', 'page-change', 'per-page-change', 'reload', 'export-started']);
    ...
    const allSelected = computed(() =>
        props.damages.length > 0 && props.damages.every(item => selectedIds.value.has(item.id))
    );

    const toggleSelectAll = () => {
        if (allSelected.value) {
            props.damages.forEach(item => selectedIds.value.delete(item.id));
            selectedIds.value = new Set(selectedIds.value);
        } else {
            const next = new Set(selectedIds.value);
            props.damages.forEach(item => next.add(item.id));
    ...
    const showingText = computed(() => {
        ...
        if (!pagination.total || pagination.total === 0) return 'No damages to show'

        const from = pagination.from || 1
        const to = pagination.to || pagination.data?.length || 0
        const total = pagination.total || 0

        return `Showing ${from} to ${to} of ${total} damages`
    });
