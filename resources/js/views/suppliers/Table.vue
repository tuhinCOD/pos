<template>
    ...
    <div class="table-responsive">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody id="list">
                <tr v-for="supplier in props.suppliers" class="text-center align-middle">
                    <td class="col-checkbox"><input type="checkbox" :checked="selectedIds.has(supplier.id)" @change="toggleSelect(supplier.id)" /></td>
                    <td style="font-size: 12px;" class="text-start">
                        <div class="about-supplier">
                            ...
                            <div>
                                <i class="bi bi-person-fill"></i>
                                Created by: {{ supplier.user?.name || '-' }}<br>
                            </div>
                            <div>
                                <i class="bi bi-person-fill"></i>
                                updated by: {{ supplier.updated_by?.name || '-' }}<br>
                            </div>
                        </div>
                    </td>
                    <td v-if="auth.user?.role?.name !== 'super admin'">
                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic mixed styles example">
                            <button type="button" class="btn btn-warning edit-btn" @click="emit('edit-supplier', supplier)"><i class="bi bi-pencil-square"></i></button>
                            <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'cashier' && auth.user?.role?.name !== 'manager' && auth.user?.role?.name !== 'warehouse staff'" class="btn btn-danger delete-btn" @click="emit('delete-supplier', supplier.id)"><i class="bi bi-trash"></i></button>
                        </div>
                    </td>
                </tr>
                <tr v-if="props.suppliers.length === 0">
...
<script setup>
    ...
    const allSelected = computed(() =>
        props.suppliers.length > 0 && props.suppliers.every(item => selectedIds.value.has(item.id))
    );

    const toggleSelectAll = () => {
        if (allSelected.value) {
            props.suppliers.forEach(item => selectedIds.value.delete(item.id));
            selectedIds.value = new Set(selectedIds.value);
        } else {
            const next = new Set(selectedIds.value);
            props.suppliers.forEach(item => next.add(item.id));
    ...
    const showingText = computed(() => {
        ...
        if (!pagination.total || pagination.total === 0) return 'No suppliers to show'

        const from = pagination.from || 1
        const to = pagination.to || pagination.data?.length || 0
        const total = pagination.total || 0

        return `Showing ${from} to ${to} of ${total} suppliers`
    })
</script>
