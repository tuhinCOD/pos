<template>
    <form id="temp-form" @submit.prevent="saveAsPending">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex align-items-center gap-2">
                <h4 class="m-0">{{ isEditing ? 'Edit Temp' : 'Point of Sale' }}</h4>
                <div class="dropdown" @click.stop>
                    <button class="btn btn-outline-secondary btn-sm position-relative" type="button" @click="togglePendingDropdown">
                        ...
                        <span v-if="pendingTemps.length" class="badge bg-danger ms-1">{{ pendingTemps.length }}</span>
                    </button>
                    <div v-if="showPendingDropdown" class="dropdown-menu show" style="min-width: 360px; max-height: 400px; overflow-y: auto; inset: 100% auto auto 0;">
                        <div class="dropdown-header d-flex justify-content-between align-items-center py-1">
                            <span class="fw-bold">Pending Invoices</span>
                        ...
                        <div v-else-if="!pendingTemps.length" class="text-center py-3 text-muted small">
                            No pending invoices.
                        </div>
                        <div
                            v-for="group in pendingTemps"
                            :key="group.invoice_no"
                            class="dropdown-item px-3 py-2 border-bottom"
                            style="cursor: pointer;"
                            @mousedown.prevent="loadInvoice(group.invoice_no)"
                        >
            ...
            <router-link to="/temps" class="btn btn-primary btn-sm">
                <i class="bi bi-eye"></i> View Temps
            </router-link>
        </div>
...
<script setup>
    ...
    const props = defineProps({
        ...
        editTemp: { type: Object, default: null },
        pendingTemps: { type: Array, default: () => [] },
    ...
    const loadInvoiceData = async () => {
        ...
        try {
            const res = await axios.get(`/api/temps/by-invoice/${field.invoice_no}`);
            const temps = res.data?.temps;
            if (temps && temps.length) {
                setPendingData(field.invoice_no, temps);
            }
        } catch {}
    };
    ...
    const loadInvoice = async (invoiceNo) => {
        try {
            const res = await axios(`/api/temps/by-invoice/${invoiceNo}`);
            if (res.data?.temps?.length) {
                setPendingData(invoiceNo, res.data.temps);
            }
        } catch {}
    ...
    const handleScanInput = () => {
        ...
        if (match) {
            ...
            if (product) {
                ...
                if (match.branch_id) field.branch = match.branch_id;
    ...
    const selectBarcodeFromSearch = (bc) => {
        ...
        if (bc.branch_id) field.branch = bc.branch_id;
    ...
    const saveAsPending = async (resetAfterSave = false) => {
        ...
        try {
                        const validationItems = cartItems.value.map(item => ({
                            ...
                            product_price: resolveProductPriceId(item.product_id, item.unit_id, item.product_price_id),
                            unit: item.unit_id || props.units[0]?.id || '',
                            qty: item.qty,
                            price: item.price,
                            vat: item.vat,
                            ...
                            ...(field.client ? { client: field.client } : {}),
                        }));
            await axios.post('/api/temps/validate-batch', { items: validationItems });
        } catch (e) {
            success = false;
        ...
        for (const item of cartItems.value) {
            ...
            try {
                if (item.temp_id) {
                    await axios.post(`/api/temps/update/${item.temp_id}`, payload);
                    savedTempIds.push(item.temp_id);
                } else {
                    const res = await axios.post('/api/temps', payload);
                    const newId = res.data?.temp?.id || res.data?.data?.id;
                    if (newId) {
                        savedTempIds.push(newId);
                        item.temp_id = newId;
                    }
                }
            } catch (e) {
        ...
        if (success) {
            for (const oldId of loadedTempIds) {
                if (!savedTempIds.includes(oldId)) {
                    try { await axios.post(`/api/temps/delete/${oldId}`); } catch {}
                }
            }

            savedInvoiceNo.value = field.invoice_no;
            loadedTempIds = savedTempIds;

            if (isCompleted) {
                const backendPaid = await fetchInvoiceTotalPaid(field.invoice_no);
                const sessionPaid = paidAmounts.value[field.invoice_no] || 0;
                const totalPaid = Math.max(backendPaid, sessionPaid);
                lastSavedStatus.value = totalPaid >= grandTotal.value ? 'completed' : 'pending';
            } else {
                lastSavedStatus.value = selectedStatus?.name || 'pending';
            }
    ...
    const loadCartFromTemps = (temps) => {
        cartItems.value = temps.map(t => ({
            ...
            point: t.point || 0,
            unit_id: t.unit_id,
            product_price_id: t.product_price_id,
            temp_id: t.id,
            attributes: normalizeAttrs(t.attributes),
        }));
        loadedTempIds = temps.map(t => t.id);
    };

    const setPendingData = (invoiceNo, temps) => {
        isEditing.value = true;
        field.invoice_no = invoiceNo;
        savedInvoiceNo.value = invoiceNo;
        if (temps.length) {
            const t = temps[0];
            field.branch = t.branch_id || t.branch?.id || '';
            field.status = t.status_id || '';
            if (t.client_id || t.client?.id) {
                field.client = t.client_id || t.client?.id;
                clientPhone.value = t.client?.contact || '';
                clientFound.value = t.client || null;
            }
        }
        loadCartFromTemps(temps);
    };
    ...
    watch(() => props.editTemp, (val) => {
        if (val) {
            ...
            field.product_price = val.product_price_id || '';
