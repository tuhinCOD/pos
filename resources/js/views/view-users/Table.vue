    ...
    <div class="table-responsive">
        ...
        <table class="table table-bordered table-striped" style="min-width: 800px;">
            ...
            <tbody id="list">
                ...
                <tr v-for="user in props.users" :key="user.id" class="text-center align-middle" style="font-size: 12px">
                    <td class="col-checkbox"><input type="checkbox" :checked="selectedIds.has(user.id)" @change="toggleSelect(user.id)" /></td>
                    ...
                    <td style="font-size: 12px;" class="text-start">
                        <div class="about-client">
                            ...
                            <div>
                                <i class="bi bi-person-fill"></i>
                                Created by: {{ user.user?.name || '-' }}<br>
                            </div>
                            <div>
                                <i class="bi bi-person-fill"></i>
                                updated by: {{ user.updated_by?.name || '-' }}<br>
                            </div>
                        </div>
                    </td>
                    <td style="font-size: 12px;" class="text-start">
                        ...
                        <div>
                            ...
                            Remarks: {{ user.remarks || '-' }}<br>
                        </div>
                        </div>
                    </td>
                    <td>
                    <div class="btn-group btn-group-sm" role="group" aria-label="Basic mixed styles example">
                        <!-- <button type="button" class="btn btn-success release-btn" id="release-btn"><i class="bi bi-eye"></i></button> -->
                        <button type="button" class="btn btn-warning edit-btn" @click="emit('edit-user', user)"><i class="bi bi-pencil-square" ></i></button>
                        <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'cashier' && auth.user?.role?.name !== 'manager' && auth.user?.role?.name !== 'warehouse staff'" class="btn btn-danger delete-btn" @click="emit('delete-user', user)"><i class="bi bi-trash"></i></button>
                    </div>
                    </td>
                </tr>
            </tbody>
...
<script setup>
    ...
    const emit = defineEmits(['edit-user', 'delete-user', 'page-change', 'per-page-change', 'reload', 'export-started']);
    ...
    const allSelected = computed(() =>
        props.users.length > 0 && props.users.every(item => selectedIds.value.has(item.id))
    );

    const toggleSelectAll = () => {
        if (allSelected.value) {
            props.users.forEach(item => selectedIds.value.delete(item.id));
            selectedIds.value = new Set(selectedIds.value);
        } else {
            const next = new Set(selectedIds.value);
            props.users.forEach(item => next.add(item.id));
    ...
    const showingText = computed(() => {
        ...
        if (!pagination.total || pagination.total === 0) return 'No clients to show'

        const from = pagination.from || 1
        const to = pagination.to || pagination.data?.length || 0
        const total = pagination.total || 0

        return `Showing ${from} to ${to} of ${total} clients`
    });
</script>
