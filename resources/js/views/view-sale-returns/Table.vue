<template>
    <div class="table-responsive">
        ...
        <table class="table table-bordered table-striped" style="min-width: 800px;">
            ...
            <tbody id="list">
                <tr v-if="!returns || returns.length === 0">
                    <td colspan="7" class="text-center text-muted">
                        No data available.
                    </td>
                </tr>
                <tr v-for="item in props.returns" :key="item.id" class="text-center align-middle" style="font-size: 12px">
                    <td class="col-checkbox"><input type="checkbox" :checked="selectedIds.has(item.id)" @change="toggleSelect(item.id)" /></td>
                    <td style="font-size: 12px;">{{ item.sale?.invoice_no || '-' }}</td>
                    ...
                    <td style="font-size: 11px;">
                        <span v-if="item.attributes && Object.keys(item.attributes).length">
                            <div v-for="(v, k) in item.attributes" :key="k">
                                <span class="fw-semibold">{{ k }}:</span> {{ v }}
                            </div>
                        </span>
                        <span v-else class="text-muted">-</span>
                    </td>
                    <td>
                        <template v-if="item.unit_id === item.product_unit_id">
                            {{ item.product_unit_qty }} {{ item.unit?.name || '' }}
                        </template>
                        <template v-else>
                            {{ item.qty }} {{ item.old_unit?.name || '' }} / {{ item.product_unit_qty }} {{ item.unit?.name || '' }}
                        </template>
                    </td>
                    <td>
                    ...
                    <div class="btn-group btn-group-sm" role="group" aria-label="Basic mixed styles example">
                        <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'warehouse staff'" type="button" class="btn btn-warning edit-btn" @click="emit('edit-return', item)" title="Edit return"><i class="bi bi-pencil-square"></i></button>
                        <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'cashier' && auth.user?.role?.name !== 'manager' && auth.user?.role?.name !== 'warehouse staff'" class="btn btn-danger delete-btn" @click="emit('delete-return', item)"><i class="bi bi-trash" title="Delete return"></i></button>
                    </div>
                    </td>
                </tr>
            </tbody>
...
<script setup>
    ...
    const emit = defineEmits(['edit-return', 'delete-return', 'page-change', 'per-page-change', 'reload', 'export-started']);
    ...
    const exportableIds = computed(() => Array.from(selectedIds.value));

    const allSelected = computed(() =>
        props.returns.length > 0 && props.returns.every(item => selectedIds.value.has(item.id))
    );

    const toggleSelectAll = () => {
        if (allSelected.value) {
            props.returns.forEach(item => selectedIds.value.delete(item.id));
            selectedIds.value = new Set(selectedIds.value);
        } else {
            const next = new Set(selectedIds.value);
            props.returns.forEach(item => next.add(item.id));
    ...
    const showingText = computed(() => {
        ...
        if (!pagination.total || pagination.total === 0) return 'No returns to show'

        const from = pagination.from || 1
        const to = pagination.to || pagination.data?.length || 0
        const total = pagination.total || 0

        return `Showing ${from} to ${to} of ${total} returns`
    });
