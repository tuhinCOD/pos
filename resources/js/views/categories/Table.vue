    ...
    <div class="table-responsive">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody id="list" style="font-size: 12px;">
                <tr v-for="category in props.categories" class="text-center align-middle">
                    <td  class="col-checkbox"><input type="checkbox" :checked="selectedIds.has(category.id)" @change="toggleSelect(category.id)" /></td>
                    <td style="font-size: 12px;" class="text-start">
                        <div class="about-category">
                            ...
                            <div>
                                ...
                                Parent: {{ category.parent?.name || '-' }}<br>
                            </div>
                            <div>
                                <i class="bi bi-person-fill"></i>
                                Created by: {{ category.user?.name || '-' }}<br>
                            </div>
                            <div>
                                <i class="bi bi-person-fill"></i>
                                updated by: {{ category.updated_by?.name || '-' }}<br>
                            </div>
                        </div>
                    </td>
                    <td v-if="auth.user?.role?.name !== 'super admin'">
                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic mixed styles example">
                            <button type="button" class="btn btn-warning edit-btn" @click="emit('edit-category', category)"><i class="bi bi-pencil-square"></i></button>
                            <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'manager'" class="btn btn-danger delete-btn" @click="emit('delete-category', category.id)"><i class="bi bi-trash"></i></button>
                        </div>
                    </td>
                </tr>
...
<script setup>
    ...
    const allSelected = computed(() =>
        props.categories.length > 0 && props.categories.every(item => selectedIds.value.has(item.id))
    );
    ...
    const confirmExportSelected = async () => {
        ...
        if (confirm(`Are you sure you want to download ${ids.length} data?`)) {
            try {
                const res = await axios.get(exportSelectedUrl.value);
    ...
    const toggleSelectAll = () => {
        if (allSelected.value) {
            props.categories.forEach(item => selectedIds.value.delete(item.id));
            selectedIds.value = new Set(selectedIds.value);
        } else {
            const next = new Set(selectedIds.value);
            props.categories.forEach(item => next.add(item.id));
    ...
    const showingText = computed(() => {
        ...
        if (!pagination.total || pagination.total === 0) return 'No categories to show'

        const from = pagination.from || 1
        const to = pagination.to || pagination.data?.length || 0
        const total = pagination.total || 0

        return `Showing ${from} to ${to} of ${total} categories`
    })
</script>
