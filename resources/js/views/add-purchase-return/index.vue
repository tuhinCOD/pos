...
<script setup>
    ...
    import { usePurchaseReturn } from "../../stores/purchaseReturn";
    import Form from "./Form.vue";

    const dash = usePurchaseReturn();
    ...
    const editReturn = computed(() => editReturnData.value || dash.allEditReturn);
    ...
    async function load() {
        await dash.loadReturns({}, 1, 20);
        if (route.params.id) {
            const result = await dash.getReturn(route.params.id);
            if (result.status === 'success') {
                editReturnData.value = result.data.supplierReturn;
            }
        }
    }
    ...
    onMounted(() => {
        if (window.Echo) {
            window.Echo.channel('supplierReturns')
                .listen('.supplier-return-updated', () => {
                    load();
                });
        }
    });
    ...
    const handleUpdateReturn = async (formData) => {
        ...
        let result;
        if (editReturn.value) {
            result = await dash.updateReturn(editReturn.value.id, formData);
        } else {
            result = await dash.createReturn(formData);
        }
    ...
    const handleReset = () => {
        ...
        if (route.params.id) {
            router.push({ name: 'purchase-return-create' });
        }
    };
</script>
