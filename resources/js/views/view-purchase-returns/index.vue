    ...
    <div class="container-fluid">
        <div class="my-3">
            <div class="card card-body">
                ...
                :perPage="perPage"
                :page="page"
                :exportEndpoint="'supplier_returns'"
...
const { handleExportStarted } = useExport('supplier_returns');
...
const returns = computed(() => dash.allReturns);
...
function load() {
    dash.loadReturns(buildApiFilters(), page.value, perPage.value);
}
...
const handleEditReturn = (item) => {
    router.push({ name: 'purchase-return-edit', params: { id: item.id } });
};

const handleDeleteReturn = async (item) => {
    if (!confirm(`Are you sure you want to delete this purchase return?`)) return;
    const result = await dash.deleteReturn(item.id, buildApiFilters(), page.value, perPage.value);
