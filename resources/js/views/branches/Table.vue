<template>
    ...
    <div class="table-responsive">
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody id="list">
                <tr v-for="branch in props.branches" class="text-center align-middle">
                    <td class="col-checkbox"><input type="checkbox" :checked="selectedIds.has(branch.id)" @change="toggleSelect(branch.id)" /></td>
                    <td style="font-size: 12px;" class="text-start">
                        <div class="about-client">
                            ...
                            <div>
                                ...
                                Created by: {{ branch.user?.name || '-' }}<br>
                            </div>
                            <div>
                                <i class="bi bi-person-fill"></i>
                                updated by: {{ branch.updated_by?.name || '-' }}<br>
                            </div>
                        </div>
                    </td>
                    <td v-if="auth.user?.role?.name !== 'super admin'">
                        <div class="btn-group btn-group-sm" role="group" aria-label="Basic mixed styles example">
                            <button type="button" class="btn btn-warning edit-btn" @click="emit('edit-branch', branch)"><i class="bi bi-pencil-square"></i></button>
                            <button class="btn btn-danger delete-btn" @click="emit('delete-branch', branch.id)"><i class="bi bi-trash"></i></button>
                        </div>
                    </td>
                </tr>
                <tr v-if="props.branches.length === 0">
...
<script setup>
    ...
    const allSelected = computed(() =>
        props.branches.length > 0 && props.branches.every(item => selectedIds.value.has(item.id))
    );
    ...
    const toggleSelectAll = () => {
        if (allSelected.value) {
            props.branches.forEach(item => selectedIds.value.delete(item.id));
            selectedIds.value = new Set(selectedIds.value);
        } else {
            const next = new Set(selectedIds.value);
            props.branches.forEach(item => next.add(item.id));
    ...
    const showingText = computed(() => {
        ...
        if (!pagination.total || pagination.total === 0) return 'No branches to show'

        const from = pagination.from || 1
        const to = pagination.to || pagination.data?.length || 0
        const total = pagination.total || 0

        return `Showing ${from} to ${to} of ${total} branches`
    })
</script>
