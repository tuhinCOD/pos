<template>
    <form id="credit-form" @submit.prevent="updateForm">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="m-0">Edit Purchase Credit</h4>
            <router-link to="/purchase-credits" class="btn btn-primary btn-sm">
        ...
        <div class="my-3">
            <div class="row g-2">
                ...
                <div class="col-4">
                    <Input v-model="field.supplier_name" label="Supplier" class="form-control" :value="field.supplier_name" disabled/>
                </div>
                <div class="col-4">
...
<script setup>
    ...
    watch(() => props.editCredit, (val) => {
        if (val) {
            field.invoice_no = val.purchase?.invoice_no || '';
            field.product_name = val.purchase?.product?.name || '';
            field.branch_name = val.purchase?.branch?.name || '';
            field.supplier_name = val.purchase?.supplier?.name || '';
    ...
    const resetForm = () => {
        if (props.editCredit) {
            field.invoice_no = props.editCredit.purchase?.invoice_no || '';
            field.product_name = props.editCredit.purchase?.product?.name || '';
            field.branch_name = props.editCredit.purchase?.branch?.name || '';
            field.supplier_name = props.editCredit.purchase?.supplier?.name || '';
