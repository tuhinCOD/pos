<template>
    <div style="overflow-x: auto;">
        <div style="min-width: 700px;">
            ...
            <div v-if="!groupedPurchases || groupedPurchases.length === 0" class="text-center text-muted py-4">
                No data available.
            </div>
            <div v-for="group in groupedPurchases" :key="group.invoice_no" class="card mb-3 shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center py-2">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="btn-group btn-group-sm" role="group">
                            ...
                            <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'cashier'" type="button" class="btn btn-warning" @click="emit('edit-purchase', group)" title="Edit invoice"><i class="bi bi-pencil-square"></i></button>
                            <button v-if="auth.user?.role?.name !== 'super admin' && auth.user?.role?.name !== 'cashier' && auth.user?.role?.name !== 'manager' && auth.user?.role?.name !== 'warehouse staff'" class="btn btn-danger" @click="emit('delete-purchase', group)"><i class="bi bi-trash" title="Delete invoice"></i></button>
                        </div>
                    </div>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive mb-0">
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                <tr v-for="purchase in group.items" :key="purchase.id" style="font-size: 12px">
                                    <td class="text-start">
                                        <div><i class="bi bi-bag-fill"></i> {{ purchase.product?.name || '-' }}</div>
                                        <div v-if="purchase.attributes && typeof purchase.attributes === 'object'" class="mt-1">
                                            <template v-if="Array.isArray(purchase.attributes)">
                                                <span v-for="attr in purchase.attributes" :key="attr.key + ':' + attr.value" class="badge bg-light text-dark me-1">
                                                    {{ attr.key }}: {{ attr.value }}
                                                </span>
                                            </template>
                                            <template v-else>
                                                <span v-for="(v, k, i) in purchase.attributes" :key="k" class="badge bg-light text-dark me-1">
                                                    {{ k }}: {{ v }}
                                                </span>
                                            </template>
                                        </div>
                                        <div><i class="bi bi-box-fill"></i> Unit: {{ purchase.unit?.name || '-' }}</div>
                                        <div><i class="bi bi-person-fill"></i> {{ purchase.supplier?.name || '-' }}</div>
                                        <div><i class="bi bi-building-fill"></i> {{ purchase.branch?.name || '-' }}</div>
                                        <div><i class="bi bi-person-badge-fill"></i> {{ purchase.user?.name || '-' }}</div>
                                        <div v-if="purchase.updated_by"><i class="bi bi-person-fill"></i> {{ purchase.updated_by?.name || '-' }}</div>
                                    </td>
                                    <td class="text-start">
                                        <div><i class="bi bi-currency-dollar"></i> Price: {{ purchase.price || '-' }}</div>
                                        <div><i class="bi bi-file-earmark-text-fill"></i> Vat: {{ purchase.vat || '-' }}</div>
                                        <div><i class="bi bi-bookmark-dash-fill"></i> Discount: {{ purchase.discount || '-' }}</div>
                                    </td>
                                    <td>{{ purchase.qty || purchase.product_unit_qty || '-' }}</td>
                                    <td>{{ (((purchase.qty || purchase.product_unit_qty) * purchase.price) + ((purchase.qty || purchase.product_unit_qty) * (purchase.vat || 0)) - ((purchase.qty || purchase.product_unit_qty) * (purchase.discount || 0))) || '-' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
<script setup>
    ...
    const props = defineProps({
        groupedPurchases: { type: Array, default: () => [] },
    ...
    const emit = defineEmits(['edit-purchase', 'delete-purchase', 'page-change', 'per-page-change', 'reload', 'export-started']);
    ...
    const getReceiptProps = (group) => {
        const items = group.items.map(purchase => ({
            product_name: purchase.product?.name || '-',
            qty: purchase.qty || purchase.product_unit_qty || 0,
            price: purchase.price,
            attributes: purchase.attributes || {}
        }));
        const subtotal = items.reduce((s, it) => s + (it.qty * it.price), 0);
        const totalVat = group.items.reduce((s, it) => s + ((it.qty || it.product_unit_qty || 0) * (it.vat || 0)), 0);
        const totalDiscount = group.items.reduce((s, it) => s + ((it.qty || it.product_unit_qty || 0) * (it.discount || 0)), 0);
        return {
            invoiceNo: group.invoice_no,
            date: group.items[0]?.created_at || '',
            branchName: group.items[0]?.branch?.name || '',
            clientName: group.items[0]?.supplier?.name || '',
            items,
            subtotal,
            totalVat,
            totalDiscount,
    ...
    const allSelected = computed(() =>
        props.groupedPurchases.length > 0 && props.groupedPurchases.every(g => selectedInvoices.value.has(g.invoice_no))
    );

    const toggleSelectAll = () => {
        if (allSelected.value) {
            props.groupedPurchases.forEach(g => selectedInvoices.value.delete(g.invoice_no));
            selectedInvoices.value = new Set(selectedInvoices.value);
        } else {
            const next = new Set(selectedInvoices.value);
            props.groupedPurchases.forEach(g => next.add(g.invoice_no));
    ...
    const showingText = computed(() => {
        ...
        if (!pagination.total || pagination.total === 0) return 'No purchases to show'

        const from = pagination.from || 1
        const to = pagination.to || pagination.data?.length || 0
        const total = pagination.total || 0

        return `Showing ${from} to ${to} of ${total} purchases`
    });
    ...
    const statusBadge = (status) => {
        const map = {
            ...
            'partial received': 'badge bg-info text-dark',
